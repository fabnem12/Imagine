...
std::complex<double> fonction(std::complex<double>& nb) {
    ...
    std::complex<double> un = std::complex<double>(1,0);
    for (int n = 0; n < 50; n++) {
        std::complex<double> puissance = pow(n, nb);
        if (puissance != std::complex<double>(0,0)) image += un / puissance;
        else continue;
    }

    if (std::isnan(real(image))) image = 0;
